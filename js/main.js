$(function(){
    const $imageprincipalOriginal = $(".imageprincipal img").attr('src');
    const $precioOriginal = $(".detail.container .price span").text();
...
$(".detail.container #empaque").on("change", function(){
    ...
    console.log("option selected " + selected_empaque.val());

    if (selected_empaque.val() == 1) {
        $(".detail.container .imageprincipal").html("<img src='"+ $imageprincipalOriginal + "' >");
        $(".detail.container .price span").html($precioOriginal);               
        $(".detail.container #tamano").val("1");
    }else{
        ...
        $.ajax({
            ...
            success:  function (response) { //una vez que el archivo recibe el request lo procesa y lo devuelve                            
               ...
               if (image_update != "") {
                $(".detail.container .imageprincipal").html("<img src='admin/uploads/products/"+ image_update + "' >");
               }
...
$(".detail.container #tamano").on("change", function(){
    ...
    if (selected_tamano.val() == 1) {
         $(".detail.container .imageprincipal").html("<img src='"+ $imageprincipalOriginal + "' >");
        $(".detail.container .price span").html($precioOriginal);
        $(".detail.container #empaque").val("1");
    }else{
        var flor_id = $('.detail.container .flor_id').text();
        const precio_producto = $(".detail.container .price_product").text();
        var parametros = {
